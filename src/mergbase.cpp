...
mergbase::mergbase(TString fileList) : fChain(0) 
{
  ifstream indata;
...
mergbase::mergbase(TString inFileName, Int_t keyID) : fChain(0) 
{
  if(keyID == 1){
    ifstream indata;
...
void mergbase::tGraphInit(TGraph *gr[nChannels], TString grName, TString grTitle){
  Int_t i;
  TString grNameh;
  TString grTitleh;
...
void mergbase::h1D1Init(TH1D *h1D1[nChannels],TString h1name, TString h1Title,
...
void mergbase::h2D2Init(TH2D *h2D1[nChannels],TString h2name, TString h2Title,
...
void mergbase::tProfInit(TProfile *tprof[nChannels],TString prname, TString prTitle,
		       Int_t Nbin, Float_t Vmin, Float_t Vmax){
  Int_t i;
  TString prnameh;
  TString prTitleh;
...
double mergbase::getUnixTimeFromTime(double d_year, double d_month, double d_day, double d_hour, double d_min, double d_sec){
  //time_t timer;
  struct tm y2k = {0};
...
void mergbase::Loop(TString histOut){
}

Int_t mergbase::GetEntry(Long64_t entry){
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}

Long64_t mergbase::LoadTree(Long64_t entry){
...
void mergbase::Show(Long64_t entry){
...
Int_t mergbase::Cut(Long64_t entry){
